...
# # Allow frontend (Vite dev server) to call backend
# CORS(app, resources={r"/api/*": {"origins": "*"}})
# # CORS(app, origins=["http://localhost:8080", "http://localhost:5173"], expose_headers=["Content-Disposition"])
# # ===== REGISTER BLUEPRINTS =====
# # IMPORTANT: prefix /api to match the frontend
# app.register_blueprint(routes, url_prefix="/api")
...
# ===== CORS — allow the Vite frontend on port 8080 =====
CORS(app, origins=["http://localhost:8080"], supports_credentials=True)

# ===== REGISTER ROUTES — no prefix =====
app.register_blueprint(routes)

@app.route("/")
def home():
    return {"message": "D-LOCK Backend API", "status": "running"}
